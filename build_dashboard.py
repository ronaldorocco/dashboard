...
function imprimirComparaPeriodos() {{
  ...
  const charts = document.getElementById('periodo-charts-wrap');
  ...
  const periodosTxt = validos.map(_periodoLabel).join('  vs  ');
  const w = window.open('','_blank','width=1000,height=760');
  w.document.write(`<!DOCTYPE html><html><head>
    <meta charset="utf-8">
    ...
    <style>
      *{{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}}
      ...
      .comp-kpi-delta{{background:white;border-radius:8px;padding:12px 14px;
    ...
    ${{kpiRow ? `<div class="kpi-row">${{kpiRow.innerHTML}}</div>` : ''}}
    ${{charts.innerHTML}}
    </body></html>`);
  w.document.close();
  w.print();
