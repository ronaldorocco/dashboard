...
/* São sempre 4 gráficos nessa linha (Tipo/Horário/Turno/Dia da Semana) —
   colunas fixas (sem media query) evitam que o 4º "quebre linha" sozinho
   em telas mais estreitas OU na impressão, o que deixava espaço vazio do
   lado dele (o grid reservava as mesmas colunas da linha de cima). Um
   breakpoint condicional aqui bagunçava a paginação do PDF (a impressão
   avalia a largura de forma diferente da tela), então ficou fixo em 4. */
...
// Ao abrir, filtra só pelo ano atual (o usuário desmarca/marca manualmente
// se quiser ver outros anos). Se o ano atual não tem dados ainda, usa o
// ano mais recente disponível.
{{
  const anosDisponiveis = [...new Set(RAW.map(r=>r.ano))];
  if(anosDisponiveis.length) {{
    const anoAtual = new Date().getFullYear();
    const anoPadrao = anosDisponiveis.includes(anoAtual) ? anoAtual : Math.max(...anosDisponiveis);
    state.ano.add(String(anoPadrao));
  }}
}}
...
function renderTurno(data) {{
  ...
  const colors = labels.map((t,i)=>vals[i]===maxTurno?COLORS.vermelho:COLORS.azulClr);
  const layout = {{...LAYOUT_BASE,
...
function renderMes(data) {{
  ...
  const colors = vals.map(v=>v===maxMes?COLORS.vermelho:COLORS.azulClr);
  const layout = {{...LAYOUT_BASE,
...
function renderHora(data) {{
  ...
  const colors = hrs.map(v => v===maxHr && v>0 ? COLORS.vermelho : COLORS.azulClr);
  const trace = [{{
...
let compAnosInit = false;

function initCompAnos(anosAll) {{
  // Primeira chamada: ativa só o ano atual (o usuário clica nos outros
  // botões pra ir comparando lado a lado).
  if (!compAnosInit && compAnosAtivos.size === 0 && anosAll.length) {{
    compAnosInit = true;
    const anoAtual  = new Date().getFullYear();
    const anoPadrao = anosAll.includes(anoAtual) ? anoAtual : Math.max(...anosAll);
    compAnosAtivos.add(anoPadrao);
  }}
...
function renderPeriodoSelector() {{
  ...
  el.innerHTML = periodos.map((p,i) => {{
    ...
    return `<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;padding:6px 10px;background:white;
      ...
      <input type="date" value="${{p.inicio}}" onchange="setPeriodoData(${{i}},'inicio',this.value)"
        style="border:1px solid #ccc;border-radius:4px;padding:3px 5px;font-size:11px;font-family:inherit;width:130px;max-width:40vw">
...
function analisarBairro() {{
  ...
  const ruasHtml = topRuasComDist.map(([nome,v,c],i) => {{
    ...
    const obs = !c ? ' · sem coordenadas (fora da rota)'
              ...
              : '';
    const avaliarBtn = c ? `<button style="margin-top:6px;font-size:10px;padding:3px 8px;border-radius:4px;
...
function _apresentRenderCharts(dataBairro, tipoCounts) {{
  ...
  const tipoTrace = {{
    type:'pie', labels:labelsT, values:valsT,
    marker:{{colors:labelsT.map(t=>TIPO_COLORS[t]||COLORS.azul), line:{{color:'#0B1220',width:2}}}},
  ...
  const maxHr = Math.max(...hrs);
  const coresH = hrs.map(v => v===maxHr && v>0 ? COLORS.vermelho : COLORS.azulClr);
  ...
  const coresTu = labelsTu.map((t,i)=>valsTu[i]===maxTurno?COLORS.vermelho:COLORS.azulClr);
  Plotly.react('apresent-chart-turno', barV(labelsTu, valsTu, coresTu), {{...darkLayout,
    ...
    yaxis:{{tickfont:{{size:9,color:'#8FA1BF'}},gridcolor:'#1E2D4A',range:[0,Math.max(...valsTu,1)*1.2]}},
  }}, CONFIG);
  ...
  Plotly.react('apresent-chart-dia', barV(labelsD, valsD, coresD), {{...darkLayout,
    ...
    yaxis:{{tickfont:{{size:9,color:'#8FA1BF'}},gridcolor:'#1E2D4A',range:[0,Math.max(...valsD,1)*1.2]}},
  }}, CONFIG);
}}
...
async function imprimirApresentacaoBairro() {{
  ...
  _apresentAplicarTamanhoImpressao(true);
  await new Promise(resolve => setTimeout(resolve, 80));

  const limpar = () => {{ _apresentAplicarTamanhoImpressao(false); window.onafterprint = null; }};
