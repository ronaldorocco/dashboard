...
function renderPlanoPolicial(data) {{
  ...
  // ── Separar os dois turnos operacionais ───────────────────────────────────
  // Turno A: 07h–19h → 8 viaturas
  // Turno B: 19h–07h → 7 viaturas
  const dadosA = data.filter(r=>{{ const h=parseInt(r.hora); return !isNaN(h)&&h>=7&&h<19; }});
  const dadosB = data.filter(r=>{{ const h=parseInt(r.hora); return !isNaN(h)&&(h>=19||h<7); }});
  const semHora= data.filter(r=>{{ const h=parseInt(r.hora); return isNaN(h); }});
