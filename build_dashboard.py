...
function abrirAnalisePredit() {{
  ...
  const bairroStats=bairroSet.map(b=>{{
    ...
    return {{b,total:recs.length,recente,med:med.toFixed(1),z,risco,pctVsMed,topTipo}};
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:18px">
    ...
    <!-- Risco por bairro -->
    <div>
      <div style="font-size:12px;font-weight:700;color:#1A1A2E;border-bottom:2px solid #7B2FBE;padding-bottom:5px;margin-bottom:8px">📊 Classificação de Risco por Bairro</div>
      <table style="width:100%;border-collapse:collapse;font-size:10px">
        <thead><tr style="background:#F0EAF8">
          ...
          <th style="padding:5px 8px;text-align:center">Ocor.</th>
          <th style="padding:5px 8px;text-align:center">vs Média</th>
          <th style="padding:5px 8px;text-align:left">Crime +freq.</th>
        </tr></thead>
        <tbody>
        ${{bairroStats.map((bs,i)=>`<tr style="background:${{i%2?'#FAFAFA':'white'}}">
          <td style="padding:4px 8px;font-weight:600">${{bs.b}}</td>
          ...
          <td style="padding:4px 8px;text-align:center;color:${{bs.pctVsMed>0?'#D13438':'#107C10'}};font-weight:700">${{bs.pctVsMed>0?'+':''}}${{bs.pctVsMed}}%</td>
          <td style="padding:4px 8px;color:#555">${{bs.topTipo}}</td>
        </tr>`).join('')}}
        </tbody>
      </table>
    </div>
    ...
    <div>
      ...
      <table style="width:100%;border-collapse:separate;border-spacing:3px;font-size:10px">
        <thead><tr>
          ...
          ${{TURNS.map(t=>`<th style="padding:4px 6px;text-align:center;font-size:10px;color:#555">${{t}}</th>`).join('')}}
        </tr></thead>
        <tbody>${{heatRows}}</tbody>
      </table>
      <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;font-size:9px;color:#777">
        <span>🟥 Alto &nbsp; 🟧 Médio-alto &nbsp; 🟨 Médio &nbsp; 🟦 Baixo</span>
      </div>
    </div>
  </div>
  ...
  <!-- Projeção -->
  <div style="margin-bottom:18px">
    <div style="font-size:12px;font-weight:700;color:#1A1A2E;border-bottom:2px solid #7B2FBE;padding-bottom:5px;margin-bottom:10px">📈 Projeção — Histórico + Previsão 90 Dias</div>
    <div style="font-size:10px;color:#888;margin-bottom:8px">Histórico recente (azul) vs projeção por IA/regressão linear (roxo tracejado)</div>
