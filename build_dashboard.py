...
async function gerarSlides() {{
  ...
  try {{
    ...
    const TXT = [60,60,66], SUBTXT = [130,130,138];
    const W = 10, H = 5.63;
    ...
    function tituloSlide(txt) {{
      ...
      doc.rect(0, 0, W, 0.58, 'F');
      doc.setFillColor(...TEAL);
      doc.rect(0, 0.58, W, 0.05, 'F');
      doc.setFillColor(...TEAL);
      doc.rect(0, 0.63, 0.07, H-0.63, 'F');
      doc.setTextColor(255,255,255);
      doc.setFont(undefined, 'bold');
      doc.setFontSize(17);
      doc.text(txt, 0.4, 0.37);
    ...
    const cardW = 2.15, cardH = 1.55, gapX = 0.2, gapY = 0.22, startX = 0.4, startY = 1.0;
    ...
    for (let i = 0; i < graficos.length; i++) {{
      ...
      try {{
        ...
        const imgW = 5.2, imgH = imgW * (450/800);
        const imgX = (W - imgW) / 2;
        const imgY = 0.82;
        doc.addImage(img, 'PNG', imgX, imgY, imgW, imgH);

        const {{ obs, sug }} = parseInsight(insightsBlocos[i]);
        let ty = imgY + imgH + 0.22;
        if (obs) {{
          doc.setFillColor(...NAVY);
          doc.rect(0.5, ty-0.1, 0.045, 0.15, 'F');
          doc.setFont(undefined, 'bold');
          doc.setFontSize(9);
          doc.setTextColor(...NAVY);
          doc.text('OBSERVAÇÃO', 0.68, ty);
          doc.setFont(undefined, 'normal');
          doc.setFontSize(9.5);
          doc.setTextColor(...TXT);
          const obsLinhas = doc.splitTextToSize(obs, 8.6);
          doc.text(obsLinhas, 0.68, ty+0.19);
          ty += 0.19 + 0.16*obsLinhas.length + 0.12;
        }}
        if (sug) {{
          doc.setFillColor(...TEAL);
          doc.rect(0.5, ty-0.1, 0.045, 0.15, 'F');
          ...
          doc.text('SUGESTÃO', 0.68, ty);
          doc.setFont(undefined, 'normal');
          doc.setFontSize(9.5);
          doc.setTextColor(...TXT);
          const sugLinhas = doc.splitTextToSize(sug, 8.6);
          doc.text(sugLinhas, 0.68, ty+0.19);
        }}
    ...
    doc.text(linhas, 0.6, 1.25, {{ lineHeightFactor:1.4 }});
