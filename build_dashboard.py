...
.chat-ia-footer{{padding:8px 12px 12px;border-top:1px solid #EEE;flex-shrink:0;text-align:center;}}
.chat-ia-footer button{{background:none;border:1px solid #D13438;color:#D13438;border-radius:6px;
  padding:6px 14px;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;}}
.chat-ia-footer button:hover{{background:#D13438;color:white;}}
...
.tech-info-box{{margin-top:20px;padding:10px 12px;background:#1A1A2E;
  border:1px solid #333;border-radius:6px;font-size:10.5px;color:#aaa;line-height:1.7;}}
.tech-info-box .tech-info-title{{font-size:11px;font-weight:700;color:var(--azul-clr);
  text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;display:block;}}
.tech-info-box b{{color:#ddd;font-weight:600;}}
...
.grafico-destacado{{animation:pulseGrafico 1s ease-in-out 2;outline:3px solid #0078D4;outline-offset:2px;}}
@keyframes pulseGrafico{{0%,100%{{box-shadow:0 2px 6px rgba(0,0,0,.07);}}50%{{box-shadow:0 0 0 8px rgba(0,120,212,.25);}}}}
...
<div class="main-layout">
  ...
  <div class="sidebar" id="sidebar">
    ...
    <div class="tech-info-box">
      <span class="tech-info-title">⚙️ Tecnologias do Sistema</span>
      <b>Linguagens:</b> Python, JavaScript, HTML/CSS, PowerShell, Batch/.bat<br>
      <b>Backend:</b> Flask + Docker<br>
      <b>Dados:</b> Pandas + openpyxl, pdfplumber, geopy<br>
      <b>IA:</b> OpenAI GPT-4o-mini, OpenAI Whisper<br>
      <b>Voz:</b> ElevenLabs, Web Audio API<br>
      <b>Gráficos:</b> Plotly<br>
      <b>Mapas:</b> Leaflet + OpenStreetMap<br>
      <b>PDF/Slides:</b> jsPDF<br>
      <b>Servidor:</b> Hostinger + EasyPanel, Vercel<br>
      <b>Versionamento:</b> GitHub + GitHub Actions<br>
      <b>Bot:</b> Telegram Bot API
    </div>
  </div>
  ...
  <div class="content">
    ...
    <div id="periodo-charts-wrap" style="display:none">
    <div class="row row2">
      <div class="chart-card">
...
function renderTabela(data) {{
  ...
  tbody.innerHTML = show.map(r => `
    <tr>
      ...
      <td>${{r.item}}</td>
      <td style="max-width:420px;overflow:hidden;text-overflow:ellipsis">${{r.descricao||''}}</td>
      <td>${{r.marca}}</td>
...
// ── RELATÓRIO DE UM DIA ESPECÍFICO ("relatório de hoje", "ocorrências do dia 15/07") ──
function montarContextoRelatorioDia(pergunta, pNorm) {{
  const hoje = new Date();
  const fmtYMD = d => `${{d.getFullYear()}}-${{String(d.getMonth()+1).padStart(2,'0')}}-${{String(d.getDate()).padStart(2,'0')}}`;
  let dataAlvo = null, rotuloData = '';

  if (/\\bhoje\\b/.test(pNorm)) {{
    dataAlvo = fmtYMD(hoje);
    rotuloData = 'hoje (' + hoje.toLocaleDateString('pt-BR') + ')';
  }} else if (/\\bontem\\b/.test(pNorm)) {{
    const ontem = new Date(hoje); ontem.setDate(ontem.getDate()-1);
    dataAlvo = fmtYMD(ontem);
    rotuloData = 'ontem (' + ontem.toLocaleDateString('pt-BR') + ')';
  }} else {{
    const m = pergunta.match(/\\b(\\d{{1,2}})\\/(\\d{{1,2}})(?:\\/(\\d{{2,4}}))?\\b/);
    if (m) {{
      ...
      dataAlvo = fmtYMD(d);
      rotuloData = d.toLocaleDateString('pt-BR');
    }}
  }}
  if (!dataAlvo) return '';

  const doDia  = RAW.filter(r => r.data === dataAlvo);
  const doDiaU = dedupBO(doDia);
  if (!doDiaU.length) {{
    return `Relatório de ${{rotuloData}}: nenhuma ocorrência registrada nessa data.`;
  }}

  const contar = (arr, campo) => {{
    const c = {{}};
    arr.forEach(r => {{ const v = r[campo]; if (v) c[v] = (c[v]||0)+1; }});
    return Object.entries(c).sort((a,b)=>b[1]-a[1]);
  }};
  const fmtLista = arr => arr.map(([k,v])=>`${{k}}: ${{v}}`).join(', ') || '(sem dados)';

  return `Relatório de ocorrências de ${{rotuloData}}:\n` +
    `Total: ${{doDiaU.length}} ocorrência(s).\n` +
    `Por tipo: ${{fmtLista(contar(doDiaU,'tipo'))}}.\n` +
    `Por bairro: ${{fmtLista(contar(doDiaU,'bairro'))}}.\n` +
    `Por turno: ${{fmtLista(contar(doDiaU,'turno'))}}.\n` +
    `Itens furtados/roubados: ${{fmtLista(contar(doDia,'item'))}}.`;
}}

// ── PREVISÃO/ANÁLISE HISTÓRICA POR DIA DA SEMANA ("previsão para sexta", "risco no sábado") ──
function montarContextoDiaSemana(pergunta, pNorm) {{
  const DIAS = ['Domingo','Segunda','Terça','Quarta','Quinta','Sexta','Sábado'];
  const diaEncontrado = DIAS.find(d => _correspondeValor(d, pNorm));
  const pedePrevisao = /previs[aã]o|preditiv|risco (para|em|de|no|na)|tend[eê]ncia/.test(pNorm);
  if (!diaEncontrado || !pedePrevisao) return '';

  const hist = dedupBO(RAW).filter(r => r.dia === diaEncontrado);
  if (!hist.length) {{
    return `Não há histórico de ocorrências registradas em ${{diaEncontrado}}s.`;
  }}
  const datas = [...new Set(hist.map(r=>r.data))].sort();
  const nDias = datas.length;
  const media = (hist.length/nDias).toFixed(1);
  ...
  let tendencia = 'poucos dados para avaliar tendência';
  if (nDias >= 4) {{
    const mid  = Math.floor(nDias/2);
    const avg1 = datas.slice(0,mid).reduce((s,d)=>s+hist.filter(r=>r.data===d).length,0)/mid;
    const avg2 = datas.slice(mid).reduce((s,d)=>s+hist.filter(r=>r.data===d).length,0)/(nDias-mid);
    const diff = avg2-avg1;
    tendencia = diff>1.5 ? 'crescente' : diff<-1.5 ? 'decrescente' : 'estável';
  }}

  return `Previsão/análise histórica para ${{diaEncontrado}}s (baseada em ${{nDias}} ${{diaEncontrado}}(s) já registrado(s) no histórico):\n` +
    `Total histórico: ${{hist.length}} ocorrência(s), média de ${{media}}/dia. Tendência: ${{tendencia}}.\n` +
    `Por tipo: ${{fmtLista(contar(hist,'tipo'))}}.\n` +
    `Por bairro (top 6): ${{contar(hist,'bairro').slice(0,6).map(([k,v])=>`${{k}}: ${{v}}`).join(', ') || '(sem dados)'}}.\n` +
    `Por turno: ${{fmtLista(contar(hist,'turno'))}}.\n` +
    `Rua mais crítica nesse dia (top 3): ${{contar(hist,'endereco').slice(0,3).map(([k,v])=>`${{k}}: ${{v}}`).join(', ') || '(sem dados)'}}.`;
}}

// ── ANÁLISE PREDITIVA GERAL (tendência mensal, projeção, alertas por bairro) ──
function montarContextoPreditivo(pNorm) {{
  const pedePreditivo = /preditiv|previs[aã]o (geral|de tend[eê]ncia)|tend[eê]ncia (geral|de crescimento)|\\balertas?\\b/.test(pNorm);
  if (!pedePreditivo) return '';
  ...
  if (mesesGlobal.length < 2) {{
    return 'Dados insuficientes (menos de 2 meses de histórico) para uma análise preditiva de tendência.';
  }}
  const contsGlobal = mesesGlobal.map(m => monGlobal[m]);
  const n = contsGlobal.length;
  const xs = contsGlobal.map((_,i)=>i);
  const mx = xs.reduce((a,b)=>a+b,0)/n, my = contsGlobal.reduce((a,b)=>a+b,0)/n;
  const num = xs.reduce((s,x,i)=>s+(x-mx)*(contsGlobal[i]-my),0);
  const den = xs.reduce((s,x)=>s+(x-mx)**2,0);
  const decl = den ? num/den : 0;
  const predizer = x => Math.max(0, Math.round(decl*x + (my - decl*mx)));
  const tendencia = decl>0.5 ? 'crescente' : decl<-0.5 ? 'decrescente' : 'estável';

  const hoje = new Date();
  const d30 = new Date(hoje); d30.setDate(d30.getDate()-30);
  const d60 = new Date(hoje); d60.setDate(d60.getDate()-60);
  const fmtYMD = d => `${{d.getFullYear()}}-${{String(d.getMonth()+1).padStart(2,'0')}}-${{String(d.getDate()).padStart(2,'0')}}`;
  const rec30 = RAWU.filter(r => r.data && r.data >= fmtYMD(d30));
  const ant30 = RAWU.filter(r => r.data && r.data >= fmtYMD(d60) && r.data < fmtYMD(d30));

  const contarBairro = arr => {{ const c = {{}}; arr.forEach(r => {{ if (r.bairro) c[r.bairro] = (c[r.bairro]||0)+1; }}); return c; }};
  const recB = contarBairro(rec30), antB = contarBairro(ant30);
  const bairros = [...new Set(RAWU.map(r=>r.bairro).filter(Boolean))];
  const alertasBairro = [];
  bairros.forEach(b => {{
    const r = recB[b]||0, a = antB[b]||0;
    if (a >= 3) {{
      const delta = Math.round((r-a)/a*100);
      if (delta > 25) alertasBairro.push(`${{b}}: alta de ${{delta}}% nos últimos 30 dias`);
      else if (delta < -25) alertasBairro.push(`${{b}}: queda de ${{Math.abs(delta)}}% nos últimos 30 dias`);
    }}
  }});

  let deltaGeral = '';
  if (ant30.length > 0) {{
    const d = Math.round((rec30.length-ant30.length)/ant30.length*100);
    deltaGeral = `${{d>0?'alta':'queda'}} de ${{Math.abs(d)}}% no total geral nos últimos 30 dias vs. os 30 dias anteriores`;
  }}

  return `Análise preditiva (regressão linear sobre ${{n}} meses de histórico):\n` +
    `Tendência mensal geral: ${{tendencia}}.\n` +
    `Projeção para os próximos 3 meses: ${{predizer(n)}}, ${{predizer(n+1)}}, ${{predizer(n+2)}} ocorrências, respectivamente.\n` +
    (deltaGeral ? `Variação recente: ${{deltaGeral}}.\n` : '') +
    `Alertas por bairro (últimos 30 dias vs. 30 dias anteriores): ${{alertasBairro.length ? alertasBairro.join('; ') : 'nenhuma variação crítica detectada'}}.`;
}}

// ── RESUMO EXECUTIVO GERAL (reaproveita o mesmo texto usado no botão "Resumo IA") ──
function montarContextoResumoGeral(pNorm) {{
  const pedeResumo = /resumo (executivo|geral|do sistema)|resumo ia|panorama geral/.test(pNorm);
  if (!pedeResumo) return '';
...
function montarContextoChat(pergunta) {{
  ...
  const contextoRelatorioDia  = montarContextoRelatorioDia(pergunta, pNorm);
  const contextoDiaSemana     = montarContextoDiaSemana(pergunta, pNorm);
  const contextoPreditivo     = montarContextoPreditivo(pNorm);
  const contextoResumoGeral   = montarContextoResumoGeral(pNorm);

  return [contextoOcorrencias, contextoAutores, contextoRelatorioDia, contextoDiaSemana, contextoPreditivo, contextoResumoGeral]
    .filter(Boolean).join('\\n\\n---\\n\\n');
}}

// A Ana se apresenta falando (não só em texto) na primeira vez que o
// painel é aberto, pra soar como o início de uma conversa de verdade.
const SAUDACAO_FALADA_ANA = 'Olá, eu sou a Ana, a assistente virtual da Guarda Municipal de Balneário Camboriú. Qual o seu nome?';
let _apresentacaoFalada = false;

function toggleChatIA() {{
  const painel = document.getElementById('chat-ia-panel');
  painel.classList.toggle('aberto');
  if (painel.classList.contains('aberto')) {{
    if (!_apresentacaoFalada) {{
      _apresentacaoFalada = true;
      // Depois que a Ana termina de se apresentar falando, já liga o
      // microfone sozinha — a pessoa não precisa clicar em nada pra responder.
      falarTexto(SAUDACAO_FALADA_ANA).then(() => {{
        _modoVozContinuo = true;
        iniciarGravacao();
      }});
    }}
  }} else {{
...
let _audioResposta = null;

function _falarTextoNavegador(texto) {{
...
// Retorna uma Promise resolvida quando a fala termina — usado pra só ligar
// o microfone de novo depois que a Ana parar de falar (evita ela "se ouvir").
async function falarTexto(texto) {{
  if (!texto) return;
  const limpo = texto.replace(/[*_#`]/g, '');
  try {{
    const resp = await fetch('/api/falar', {{
      method: 'POST',
      headers: {{'Content-Type': 'application/json'}},
      body: JSON.stringify({{texto: limpo}})
    }});
    if (!resp.ok) throw new Error('falha na síntese de voz');
...
// ── "MOSTRAR GRÁFICO X" — rola a tela até o gráfico pedido e destaca ──────────
// Os gráficos já existem na tela (Plotly), a Ana só precisa apontar pra eles.
const MAPA_GRAFICOS = [
  {{ id:'chart-heatmap', termos:['mapa de calor','heatmap'] }},
  {{ id:'chart-hora',    termos:['distribuicao por hora','por hora','horario do dia'] }},
  {{ id:'chart-dia',     termos:['dia da semana'] }},
  {{ id:'chart-linha',   termos:['ocorrencias por dia','serie temporal','linha do tempo','evolucao diaria'] }},
  {{ id:'chart-mes',     termos:['por mes','mensal'] }},
  {{ id:'chart-tipo',    termos:['tipificacao','tipo de ocorrencia','tipos de ocorrencia','tipo de crime'] }},
  {{ id:'chart-item',    termos:['item mais furtado','itens furtados','itens roubados','itens mais furtados'] }},
  {{ id:'chart-refs',    termos:['ponto de referencia','pontos de referencia'] }},
  {{ id:'chart-ruas',    termos:['rua','ruas'] }},
  {{ id:'chart-bairro',  termos:['bairro','bairros'] }},
  {{ id:'chart-turno',   termos:['turno'] }},
];

function localizarGraficoPedido(pNorm) {{
  const pedeGrafico = /mostr[ae]|exib[ei]|\\babre\\b|\\babrir\\b|gr[aá]fico/.test(pNorm);
  if (!pedeGrafico) return null;
  for (const {{id, termos}} of MAPA_GRAFICOS) {{
    if (termos.some(t => pNorm.includes(t))) return id;
  }}
  return null;
}}

function destacarGrafico(id) {{
  const el = document.getElementById(id);
  if (!el) return false;
  const card = el.closest('.chart-card') || el;
  card.scrollIntoView({{ behavior:'smooth', block:'center' }});
  card.classList.add('grafico-destacado');
  setTimeout(() => card.classList.remove('grafico-destacado'), 2500);
  return true;
}}

// Histórico da conversa (só nesta sessão do navegador) — permite a Ana
// lembrar se já perguntou o nome e manter a continuidade do diálogo.
let _chatHistorico = [];
...
async function enviarPerguntaChat() {{
  ...
  try {{
    const resp = await fetch('/api/chat', {{
      method: 'POST',
      headers: {{'Content-Type': 'application/json'}},
      body: JSON.stringify({{pergunta, contexto, historico: _chatHistorico}})
    }});
    const data = await resp.json();
    if (!resp.ok) throw new Error(data.erro || 'Erro desconhecido');
...
// ── ENTRADA POR VOZ (Whisper via backend) — com detecção automática de silêncio ──
// Depois que a Ana fala, o microfone liga sozinho (_modoVozContinuo); a
// pessoa só precisa falar — quando parar de falar por um instante, a
// gravação é enviada automaticamente, sem precisar clicar em nada.
let _mediaRecorder   = null;
let _audioChunks     = [];
let _gravando        = false;
let _modoVozContinuo = false;
let _audioCtxGravacao = null;

function _pararGravacaoInterna() {{
  if (!_gravando || !_mediaRecorder) return;
  _gravando = false;
  const btn = document.getElementById('chat-ia-mic');
  if (btn) {{ btn.textContent = '🎤'; btn.classList.remove('gravando'); }}
  _mediaRecorder.stop();
}}
...
async function iniciarGravacao() {{
  ...
  try {{
    ...
    _mediaRecorder.onstop = async () => {{
      ...
      if (_audioCtxGravacao) {{ _audioCtxGravacao.close().catch(()=>{{}}); _audioCtxGravacao = null; }}
      const blob = new Blob(_audioChunks, {{ type: 'audio/webm' }});
      // Grava vazia/silêncio (ninguém falou) — encerra o modo contínuo em
      // silêncio, em vez de mandar áudio vazio pra IA e ficar num loop.
      if (blob.size < 3000) {{ _modoVozContinuo = false; return; }}
      await transcreverAudio(blob);
    }};
    _mediaRecorder.start();
    _gravando = true;
    if (btn) {{ btn.textContent = '⏹'; btn.classList.add('gravando'); }}

    // Detecção de silêncio via Web Audio API — só ativa no modo contínuo
    // (o clique manual no microfone continua parando só quando clicar de novo).
    if (_modoVozContinuo) {{
      _audioCtxGravacao = new (window.AudioContext || window.webkitAudioContext)();
      const source   = _audioCtxGravacao.createMediaStreamSource(stream);
      const analyser = _audioCtxGravacao.createAnalyser();
      analyser.fftSize = 512;
      source.connect(analyser);
      const dados = new Uint8Array(analyser.frequencyBinCount);
      const LIMIAR_VOLUME = 12, SILENCIO_MS = 1600, TEMPO_MAX_MS = 15000;
      const inicio = Date.now();
      let falouAlgumaVez = false, silencioDesde = null;

      (function checar() {{
        if (!_gravando) return;
        analyser.getByteFrequencyData(dados);
        const volume = dados.reduce((s,v)=>s+v,0)/dados.length;
        if (volume > LIMIAR_VOLUME) {{
          falouAlgumaVez = true;
          silencioDesde = null;
        }} else if (falouAlgumaVez) {{
          if (silencioDesde === null) silencioDesde = Date.now();
          if (Date.now()-silencioDesde > SILENCIO_MS) {{ _pararGravacaoInterna(); return; }}
        }}
        if (Date.now()-inicio > TEMPO_MAX_MS) {{ _pararGravacaoInterna(); return; }}
...
function toggleGravacao() {{
  if (_gravando) {{
    _modoVozContinuo = false;
    _pararGravacaoInterna();
  }} else {{
    _modoVozContinuo = false; // clique manual: para quando clicar de novo, não por silêncio
    iniciarGravacao();
  }}
}}
...
function buildSidebar() {{
  ...
  // O número ao lado do checkbox de item agora é por ocorrência (B.O.), igual
  // a todos os outros filtros — antes contava cada linha de item separado
  // (RAW), o que divergia do card "Total de Ocorrências" ao aplicar o filtro
  // e confundia quem comparava com o bot do Telegram. O gráfico "10 Itens Mais
  // Furtados/Roubados" (renderItem) continua com a contagem bruta por item,
  // que é a métrica certa ali (quantidade de itens levados, não de boletins).
  buildCheckboxes('filter-item',       'item',        itens,      ocCounts('item'));
...
function imprimirComparaPeriodos() {{
  if (!_periodoCache) {{
    alert('Selecione as datas de ao menos um período antes de gerar o PDF.');
    return;
  }}
  const {{ dadosPorPeriodo, nomes, cores, periodosTxt, kpiHTML }} = _periodoCache;
  ...
  const {{traces:bairroTraces}} = _periodoTraceBairro(dadosPorPeriodo,nomes,cores);

  const w = window.open('','_blank','width=700,height=900');
  w.document.write(`<!DOCTYPE html><html><head>
    <meta charset="utf-8">
    <title>Comparação de Períodos — Guarda Municipal BC</title>
    <style>
      *{{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}}
      @page{{size:A4;margin:15mm;}}
      body{{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;color:#222;margin:0;}}
      .pdf-page{{padding:10px 0;}}
    ...
    <div class="pdf-page">
      ...
      <div class="chart-card"><div class="chart-title">📊 Por Tipificação</div><div id="pdf-chart-tipo"></div></div>
      <div class="chart-card"><div class="chart-title">🕐 Por Turno</div><div id="pdf-chart-turno"></div></div>
    </div>
    <div class="pdf-page">
      <div class="chart-card"><div class="chart-title">📍 Por Bairro — Top 10</div><div id="pdf-chart-bairro"></div></div>
      <div class="chart-card"><div class="chart-title">📆 Dia da Semana</div><div id="pdf-chart-dia"></div></div>
    </div>
    </body></html>`);
  w.document.close();

  Promise.all([
    Plotly.newPlot(w.document.getElementById('pdf-chart-tipo'),
      _periodoTraceCategorico('tipo',TIPOS_ORD2,dadosPorPeriodo,nomes,cores),
      {{...LCOMP_PDF, xaxis:{{tickfont:{{size:9}},tickangle:-15}}, yaxis:{{showgrid:true,gridcolor:'#F0F0F0',zeroline:false,tickfont:{{size:8}}}}, margin:{{l:30,r:10,t:36,b:55}}}},
      {{staticPlot:true}}),
    Plotly.newPlot(w.document.getElementById('pdf-chart-turno'),
      _periodoTraceCategorico('turno',TURN_ORD2,dadosPorPeriodo,nomes,cores),
      ...
      {{staticPlot:true}}),
    Plotly.newPlot(w.document.getElementById('pdf-chart-bairro'), bairroTraces,
      {{...LCOMP_PDF, height:ALT_BAIRRO, xaxis:{{showgrid:true,gridcolor:'#F0F0F0',zeroline:false,tickfont:{{size:8}}}}, yaxis:{{tickfont:{{size:9}},automargin:true}}, margin:{{l:95,r:30,t:36,b:24}}}},
      {{staticPlot:true}}),
...
function renderPeriodoSelector() {{
  ...
  el.innerHTML = periodos.map((p,i) => {{
    ...
    + `<button onclick="imprimirComparaPeriodos()" class="comp-ano-btn" style="background:#107C10;color:white;border:2px solid #107C10" title="Gerar PDF só desta comparação">🖨️ Gerar PDF</button>`;
}}

// Traços reutilizados tanto pelos gráficos ao vivo quanto pelo PDF (que os
// redesenha do zero com largura fixa, em vez de clonar o SVG já renderizado).
function _periodoTraceCategorico(campo, categorias, dadosPorPeriodo, nomes, cores) {{
...
function _periodoTraceBairro(dadosPorPeriodo, nomes, cores) {{
  const bairroTotais = {{}};
  ...
  return {{
    top10B2,
    traces: dadosPorPeriodo.map((d,i) => ({{
...
// Guarda os últimos dados calculados para o PDF poder redesenhar os gráficos
// com largura fixa (evita cortes/estouros que dependiam do tamanho de tela).
let _periodoCache = null;
...
function renderComparaPeriodos() {{
  ...
  const chartsWrap = document.getElementById('periodo-charts-wrap');
  ...
  const TIPOS_ORD2 = ['Furto','Roubo','Arrombamento','Tentativa de Furto','Tentativa de Roubo'];
  Plotly.react('periodo-chart-tipo', _periodoTraceCategorico('tipo',TIPOS_ORD2,dadosPorPeriodo,nomes,cores),
  ...
  const TURN_ORD2 = ['Manhã','Tarde','Noite','Madrugada'];
  Plotly.react('periodo-chart-turno', _periodoTraceCategorico('turno',TURN_ORD2,dadosPorPeriodo,nomes,cores),
  ...
  Plotly.react('periodo-chart-dia', _periodoTraceCategorico('dia',DIA_ORDER,dadosPorPeriodo,nomes,cores),
    {{...LCOMP, xaxis:{{tickfont:{{size:10}}}}, yaxis:{{showgrid:true,gridcolor:'#F0F0F0',zeroline:false,tickfont:{{size:9}}}}, margin:{{l:30,r:12,t:44,b:36}}}}, CONFIG);

  const {{traces:bairroTraces}} = _periodoTraceBairro(dadosPorPeriodo,nomes,cores);
  Plotly.react('periodo-chart-bairro', bairroTraces,
  ...
  _periodoCache = {{
    dadosPorPeriodo, nomes, cores,
    periodosTxt: nomes.join('  vs  '),
    kpiHTML: kpiRow ? kpiRow.innerHTML : '',
  }};
}}
...
let _planoOperacional = [];
let _planoMarkers = null;
...
function analisarBairro() {{
  ...
  // Centro geográfico do bairro (mediana, robusta a outliers) — usado pra
  // descartar ruas com geocode errado (endereço ambíguo caiu longe do resto,
  // ex: mesmo nome de rua em outro bairro da cidade), que puxariam a rota
  // de patrulhamento pra fora do bairro selecionado.
  const coordsBairro = dataBairro.filter(r => r.lat && r.lon);
  const RAIO_MAX_M = 2000;
  const centroLat = _mediana(coordsBairro.map(r=>r.lat));
  const centroLon = _mediana(coordsBairro.map(r=>r.lon));

  const topRuasComDist = topRuas.map(([nome,v]) => {{
    if(v.lats.length === 0) return [nome, v, null];
    const lat = v.lats.reduce((a,b)=>a+b,0)/v.lats.length;
    const lon = v.lons.reduce((a,b)=>a+b,0)/v.lons.length;
    const dist = coordsBairro.length ? _distanciaMetros(centroLat, centroLon, lat, lon) : 0;
    return [nome, v, {{lat, lon, dist}}];
  }});

  const waypoints = topRuasComDist
    .filter(([,,c]) => c && c.dist <= RAIO_MAX_M)
    .map(([nome,v,c]) => ({{ nome, count:v.count, lat:c.lat, lon:c.lon }}));
  ultimaAnaliseBairro = {{
  ...
  window._ruasBairroAtual = topRuasComDist.map(([nome,v,c]) => (c && c.dist <= RAIO_MAX_M) ? {{nome, lat:c.lat, lon:c.lon, count:v.count}} : null);
  _planoOperacional = [];
  if(_planoMarkers && mapaInst) {{ mapaInst.removeLayer(_planoMarkers); _planoMarkers = null; }}
  ...
  const ruasHtml = topRuasComDist.map(([nome,v,c],i) => {{
    ...
    const obs = !c ? ' · sem coordenadas (fora da rota)'
              ...
              : '';
    // Só oferece foto/atribuição de viatura com coordenada confiável — numa
    // rua com geocode suspeito, a foto e o marcador apontariam pro lugar
    // errado, o que é um risco real quando a viatura é despachada de verdade.
    const coordConfiavel = c && c.dist <= RAIO_MAX_M;
    const acoesBtn = coordConfiavel ? `<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px">
      <button style="font-size:10px;padding:3px 8px;border-radius:4px;
        border:1px solid #0078D4;background:white;color:#0078D4;cursor:pointer"
        onclick="toggleTriagemRua(${{i}})">📷 Avaliar rua</button>
      <span id="triagem-badge-${{i}}"></span>
      <button style="font-size:10px;padding:3px 8px;border-radius:4px;
        border:1px solid #003D66;background:white;color:#003D66;cursor:pointer"
        onclick="toggleAtribuirViatura(${{i}})">🚓 Atribuir viatura</button>
      </div>
      <div id="triagem-painel-${{i}}" style="display:none;margin-top:8px"></div>
      <div id="viatura-painel-${{i}}" style="display:none;margin-top:8px"></div>` : '';
    return `<div class="posto-item">
      <div class="posto-num">${{i+1}}</div>
      <div class="posto-info">
        ...
        <div class="posto-detalhe">${{v.count}} ocorrência${{v.count>1?'s':''}}${{obs}}</div>
  ...
  cont.innerHTML = `
    ...
    <div class="plano-card" id="plano-operacional-card" style="display:none;margin-top:12px">
      <div class="plano-head" style="background:linear-gradient(135deg,#003D66,#0078D4)">🚓 Plano de Pontos Fixos da Operação — ${{bairro}}</div>
      <div class="plano-body" id="plano-operacional-lista"></div>
      <div style="padding:0 14px 12px;display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn-analise" onclick="compartilharPlanoOperacional()">📱 Compartilhar via WhatsApp</button>
        <button class="btn-pdf" onclick="limparPlanoOperacional()">✖ Limpar Plano</button>
      </div>
    </div>`;

  carregarTriagemBairro(bairro);
}}

// ── TRIAGEM URBANA (avaliação manual assistida pela foto do Street View) ─────
// Banco próprio da guarda: o analista olha a foto e marca o que vê. Não é
// classificação automática por IA em cima da imagem do Google (isso não é
// permitido pelos termos de uso) — é decisão humana, registrada num banco
// nosso, servida pelo backend (app.py → /api/triagem).
let _triagemCache = {{}};
const TRIAGEM_CAMPOS = [
  ['terreno_baldio',      '🌿 Terreno baldio'],
  ['imovel_abandonado',   '🏚️ Imóvel abandonado'],
  ['iluminacao_ruim',     '💡 Iluminação insuficiente'],
  ['muro_baixo',          '🧱 Muro baixo/sem proteção'],
  ['sem_cerca',           '🚪 Sem cerca/acesso vulnerável'],
  ['vegetacao_alta',      '🌳 Vegetação prejudica visibilidade'],
];

function _triagemBadgeHtml() {{
...
function toggleTriagemRua(idx) {{
  const painel = document.getElementById('triagem-painel-'+idx);
  if(!painel) return;
  ...
  const chave = `${{bairro}}|${{r.nome}}`;
  const existente = _triagemCache[chave] || {{}};

  const checks = TRIAGEM_CAMPOS.map(([campo,label]) => `
    <label style="display:flex;align-items:center;gap:4px;background:#F5F5F5;padding:3px 8px;border-radius:12px;cursor:pointer;font-size:10.5px">
      <input type="checkbox" id="triagem-${{campo}}-${{idx}}" ${{existente[campo]?'checked':''}}> ${{label}}
    </label>`).join('');
  ...
  painel.innerHTML = `
    ...
    <textarea id="triagem-obs-${{idx}}" placeholder="Observações..."
      style="width:100%;max-width:280px;min-height:44px;font-size:11px;padding:6px;border-radius:6px;border:1px solid #DDD;box-sizing:border-box">${{existente.observacoes||''}}</textarea>
...
async function salvarTriagemRua(idx) {{
  const r = window._ruasBairroAtual[idx];
  const bairro = ultimaAnaliseBairro.bairro;
  const corpo = {{ bairro, endereco: r.nome, observacoes: document.getElementById('triagem-obs-'+idx).value }};
  TRIAGEM_CAMPOS.forEach(([campo]) => {{ corpo[campo] = document.getElementById(`triagem-${{campo}}-${{idx}}`).checked; }});

  const status = document.getElementById('triagem-status-'+idx);
  ...
  try {{
    ...
    status.textContent = 'Salvo em ' + new Date().toLocaleString('pt-BR');
...
// ── PLANO DE PONTOS FIXOS (viaturas atribuídas pelo supervisor p/ operação) ──
// Fica só na sessão do navegador (não é persistido no servidor) — é um plano
// operacional pontual, feito pra ser compartilhado com a equipe na hora.
function toggleAtribuirViatura(idx) {{
  const painel = document.getElementById('viatura-painel-'+idx);
  if(!painel) return;
  if(painel.style.display !== 'none') {{ painel.style.display = 'none'; return; }}
  painel.style.display = 'block';
  if(painel.dataset.built) return;
  painel.dataset.built = '1';
  painel.innerHTML = `
    <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
      <input id="viatura-id-${{idx}}" placeholder="Viatura (ex: VTR-05)"
        style="font-size:11px;padding:5px 8px;border-radius:5px;border:1px solid #DDD;width:130px">
      <input id="viatura-min-${{idx}}" type="number" min="1" placeholder="Minutos" value="15"
        style="font-size:11px;padding:5px 8px;border-radius:5px;border:1px solid #DDD;width:80px">
      <button class="btn-analise" style="font-size:10px;padding:5px 10px" onclick="adicionarViaturaRua(${{idx}})">➕ Adicionar</button>
    </div>`;
}}

function adicionarViaturaRua(idx) {{
  const r = window._ruasBairroAtual[idx];
  const viatura = document.getElementById('viatura-id-'+idx).value.trim();
  const minutos = parseInt(document.getElementById('viatura-min-'+idx).value, 10);
  if(!viatura) {{ alert('Informe a identificação da viatura.'); return; }}
  if(!minutos || minutos <= 0) {{ alert('Informe um tempo em minutos válido.'); return; }}

  _planoOperacional.push({{ nome:r.nome, lat:r.lat, lon:r.lon, viatura, minutos }});
  document.getElementById('viatura-id-'+idx).value = '';
  renderPlanoOperacional();
}}

function removerPontoOperacional(i) {{
  _planoOperacional.splice(i,1);
  renderPlanoOperacional();
}}

function renderPlanoOperacional() {{
  const card = document.getElementById('plano-operacional-card');
  const lista = document.getElementById('plano-operacional-lista');
  if(!card || !lista) return;

  if(_planoOperacional.length === 0) {{
  ...
  lista.innerHTML = _planoOperacional.map((p,i) => `
    <div class="posto-item">
      <div class="posto-num" style="background:#003D66">🚓</div>
      <div class="posto-info">
        <div class="posto-local">${{p.viatura}} — ${{p.nome}}</div>
        <div class="posto-detalhe">${{p.minutos}} minuto${{p.minutos>1?'s':''}} neste ponto</div>
      </div>
      <button style="border:none;background:none;color:#D13438;font-size:14px;cursor:pointer;padding:0 6px"
        onclick="removerPontoOperacional(${{i}})" title="Remover">✖</button>
    </div>`).join('');

  initMapa();
  if(_planoMarkers) mapaInst.removeLayer(_planoMarkers);
  _planoMarkers = L.layerGroup(_planoOperacional.map(p => L.marker([p.lat,p.lon], {{
    icon: L.divIcon({{
      html:`<div style="background:#0078D4;color:white;border-radius:50%;width:30px;height:30px;
        display:flex;align-items:center;justify-content:center;font-size:15px;
        border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,.4)">🚓</div>`,
      className:'', iconSize:[30,30], iconAnchor:[15,15],
    }})
  }}).bindPopup(`<strong>${{p.viatura}}</strong><br>${{p.nome}}<br>${{p.minutos}} min neste ponto`))).addTo(mapaInst);
}}

function limparPlanoOperacional() {{
  _planoOperacional = [];
  renderPlanoOperacional();
}}

function compartilharPlanoOperacional() {{
  if(!ultimaAnaliseBairro || _planoOperacional.length === 0) return;
  const agora = new Date();
  const linhas = [
    `🚓 *PLANO DE PONTOS FIXOS — ${{ultimaAnaliseBairro.bairro}}*`,
    `📅 ${{agora.toLocaleDateString('pt-BR')}} ${{agora.toLocaleTimeString('pt-BR',{{hour:'2-digit',minute:'2-digit'}})}}`,
    ``,
    ..._planoOperacional.map((p,i) => `${{i+1}}. *${{p.viatura}}* — ${{p.nome}} — ${{p.minutos}} min`),
    ``,
    `_Guarda Municipal de Balneário Camboriú_`,
  ];
...
<div id="chat-ia-panel" class="chat-ia-panel">
  ...
  <div class="chat-ia-footer">
    <button onclick="encerrarConversaChat()">Encerrar conversa</button>
  </div>
</div>
<!-- ── MODAL PREVISÃO ── -->
