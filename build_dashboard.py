...
.header h1{{font-size:16px;font-weight:700;letter-spacing:.3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
...
@media(max-width:768px){{
  ...
  .header-right{{width:100%;gap:4px;flex-wrap:nowrap;justify-content:stretch;}}
  .header-right button{{padding:8px 2px;font-size:10px;flex:1 1 0;min-width:0;text-align:center;}}
  .badge{{display:none;}}
...
<div class="main-layout">
  ...
  <div class="content">
    ...
    <!-- ANÁLISE PREDITIVA -->
    <div class="section-header">🔮 Análise Preditiva
      <span style="font-size:10px;font-weight:400;color:#888;margin-left:8px">Validação automática: previsão do modelo vs. ocorrências reais</span>
    </div>
    <div class="kpi-row" id="pred-kpi-row" style="display:none"></div>
    <div class="row row2">
      <div class="chart-card">
        <div class="chart-title"><span class="icon">🎯</span> Previsto vs. Real — Histórico Diário</div>
        <div id="pred-subtit-pvr" style="font-size:10px;color:#999;margin:-6px 0 6px"></div>
        <div id="pred-chart-pvr" style="height:240px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title"><span class="icon">📊</span> Acurácia por Dia da Semana</div>
        <div id="pred-subtit-dia" style="font-size:10px;color:#999;margin:-6px 0 6px"></div>
        <div id="pred-chart-dia" style="height:240px"></div>
      </div>
    </div>
    <div class="row row2">
      <div class="chart-card">
        <div class="chart-title"><span class="icon">🗓️</span> Mapa de Risco — Dia × Turno</div>
        <div id="pred-chart-heatmap" style="height:260px"></div>
      </div>
      <div class="chart-card">
        <div class="chart-title"><span class="icon">📋</span> Últimas Previsões vs. Real</div>
        <div id="pred-subtit-tab" style="font-size:10px;color:#999;margin:-6px 0 6px"></div>
        <div id="pred-tabela" style="max-height:260px;overflow-y:auto"></div>
      </div>
    </div>
...
function renderLinha(data) {{
  ...
  if(anos.length <= 1) {{
    // ── Um único ano: comportamento original ──────────────────────────────────
    const c = {{}};
    ...
    const layout = {{...LAYOUT_BASE,
      xaxis:{{tickformat:'%d/%m',tickfont:{{size:9}},dtick:'D2',showgrid:false}},
    ...
    // ── Múltiplos anos: uma linha por ano, eixo = mês/dia alinhado ────────────
    const traces = anos.map((a,i) => {{
      const c = {{}};
      data.filter(r=>r.ano===a&&r.data).forEach(r => {{
        const key = '2000-' + r.data.slice(5); // '2000-MM-DD'
        c[key] = (c[key]||0)+1;
      }});
      const dates = Object.keys(c).sort();
      const vals  = dates.map(d=>c[d]);
...
// ── ANÁLISE PREDITIVA ────────────────────────────────────────────────────────
function renderPreditivo(data) {{
  if(typeof Plotly === 'undefined') return;
  const DIAS_ORD = ['Segunda','Terça','Quarta','Quinta','Sexta','Sábado','Domingo'];
  const TURNS    = ['MANHÃ','TARDE','NOITE'];
  const COR_ACERTO = 'rgba(16,124,16,0.75)';
  const COR_ERRO   = 'rgba(209,52,56,0.75)';
  const COR_BAND   = 'rgba(224,123,0,0.18)';

  // ── Agrupa por data ─────────────────────────────────────────────────────────
  const byDate = {{}};
  data.forEach(r => {{
    if(!r.data) return;
    if(!byDate[r.data]) byDate[r.data] = {{count:0, dia:r.dia||'', tipos:{{}}, turnos:{{}}}};
    byDate[r.data].count++;
    if(r.tipo)  byDate[r.data].tipos[r.tipo]  = (byDate[r.data].tipos[r.tipo] ||0)+1;
    if(r.turno) byDate[r.data].turnos[r.turno]= (byDate[r.data].turnos[r.turno]||0)+1;
  }});
  const dates = Object.keys(byDate).sort();
  if(dates.length < 2) {{
    ['pred-kpi-row','pred-chart-pvr','pred-chart-dia','pred-chart-heatmap','pred-tabela']
      .forEach(id=>{{ const el=document.getElementById(id); if(el) el.innerHTML=''; }});
    return;
  }}

  // ── LOO (Leave-One-Out): para cada data, previsão sem ela mesma ─────────────
  const preds = dates.map(d => {{
    const dia = byDate[d].dia;
    const iguais = dates.filter(x => x!==d && byDate[x].dia===dia).map(x=>byDate[x].count);
    if(iguais.length === 0) return {{date:d, actual:byDate[d].count, predMin:null, predMax:null, predMed:null, acerto:null, dia}};
    const med  = iguais.reduce((a,b)=>a+b,0)/iguais.length;
    const std  = Math.sqrt(iguais.reduce((s,x)=>s+Math.pow(x-med,2),0)/iguais.length);
    const pMin = Math.max(0, Math.round(med-std));
    const pMax = Math.round(med+std);
    const ac   = byDate[d].count >= pMin && byDate[d].count <= pMax;
    return {{date:d, actual:byDate[d].count, predMin:pMin, predMax:pMax, predMed:+med.toFixed(1), acerto:ac, dia, tipos:byDate[d].tipos}};
  }});

  const valid   = preds.filter(p=>p.predMin!==null);
  const nAcertos= valid.filter(p=>p.acerto).length;
  const acurPct = valid.length>0 ? Math.round(nAcertos/valid.length*100) : 0;
  const erroMed = valid.length>0 ? (valid.reduce((s,p)=>s+Math.abs(p.actual-p.predMed),0)/valid.length).toFixed(1) : 0;
  const tendDir = valid.length>=4 ? (()=>{{ const h=Math.floor(valid.length/2); const a1=valid.slice(0,h),a2=valid.slice(h); const m1=a1.reduce((s,p)=>s+p.actual,0)/a1.length, m2=a2.reduce((s,p)=>s+p.actual,0)/a2.length; return m2>m1?'▲ Crescente':'▼ Decrescente'; }})() : 'Estável';

  // ── KPIs ────────────────────────────────────────────────────────────────────
  const kpiEl = document.getElementById('pred-kpi-row');
  if(kpiEl) {{
    const acurCor  = acurPct>=70?'#107C10':acurPct>=50?'#E07B00':'#D13438';
    const tendCor  = tendDir.includes('▲')?'#D13438':tendDir.includes('▼')?'#107C10':'#555';
    const topTipo  = Object.entries(data.reduce((m,r)=>{{ if(r.tipo)m[r.tipo]=(m[r.tipo]||0)+1; return m; }},{{}})).sort((a,b)=>b[1]-a[1])[0];
    const topTurno = Object.entries(data.reduce((m,r)=>{{ if(r.turno)m[r.turno]=(m[r.turno]||0)+1; return m; }},{{}})).sort((a,b)=>b[1]-a[1])[0];
    kpiEl.innerHTML =
      `<div class="kpi-card" style="border-color:${{acurCor}}"><div class="kpi-val" style="color:${{acurCor}}">${{acurPct}}%</div><div class="kpi-label">Acurácia do Modelo</div><div class="kpi-sub">${{nAcertos}} de ${{valid.length}} dias no intervalo</div></div>` +
      `<div class="kpi-card" style="border-color:#888"><div class="kpi-val" style="color:#555;font-size:22px">${{erroMed}}</div><div class="kpi-label">Erro Médio (ocorrências/dia)</div><div class="kpi-sub">Diferença entre previsto e real</div></div>` +
      `<div class="kpi-card" style="border-color:${{tendCor}}"><div class="kpi-val" style="color:${{tendCor}};font-size:20px">${{tendDir}}</div><div class="kpi-label">Tendência Geral</div><div class="kpi-sub">Comparando períodos</div></div>` +
      (topTipo ? `<div class="kpi-card laranja"><div class="kpi-val">${{topTipo[0]}}</div><div class="kpi-label">Crime Mais Frequente</div><div class="kpi-sub">${{topTipo[1]}} ocorrências</div></div>` : '') +
      (topTurno? `<div class="kpi-card teal"><div class="kpi-val">${{topTurno[0]}}</div><div class="kpi-label">Turno Mais Crítico</div><div class="kpi-sub">${{topTurno[1]}} ocorrências</div></div>` : '');
    kpiEl.style.display = '';
  }}

  // ── Chart 1: Previsto vs Real — linha temporal ───────────────────────────────
  const elPvr = document.getElementById('pred-chart-pvr');
  if(elPvr && valid.length>0) {{
    const rec = valid.slice(-60); // últimas 60 datas
    const xDt  = rec.map(p=>p.date);
    const yAct = rec.map(p=>p.actual);
    const yMed = rec.map(p=>p.predMed);
    const yHi  = rec.map(p=>p.predMax);
    const yLo  = rec.map(p=>p.predMin);
    const barColors = rec.map(p=>p.acerto?COR_ACERTO:COR_ERRO);

    const sub = document.getElementById('pred-subtit-pvr');
    if(sub) sub.textContent = 'Verde = dentro do intervalo previsto | Vermelho = fora do intervalo';

    Plotly.react('pred-chart-pvr',[
      {{type:'scatter',mode:'lines',name:'Lim. sup.',x:xDt,y:yHi,line:{{width:0}},showlegend:false,hoverinfo:'skip'}},
      {{type:'scatter',mode:'lines',name:'Intervalo previsto',x:xDt,y:yLo,fill:'tonexty',fillcolor:COR_BAND,line:{{width:0}},hoverinfo:'skip'}},
      {{type:'scatter',mode:'lines',name:'Previsão (média)',x:xDt,y:yMed,line:{{color:'#E07B00',width:1.5,dash:'dash'}},hovertemplate:'<b>%{{x}}</b><br>Previsto: %{{y}}<extra></extra>'}},
      {{type:'bar',name:'Real',x:xDt,y:yAct,marker:{{color:barColors}},hovertemplate:'<b>%{{x}}</b><br>Real: %{{y}}<extra></extra>'}},
    ],{{
      ...LAYOUT_BASE, barmode:'overlay',
      showlegend:true, legend:{{orientation:'h',x:0,y:1.22,font:{{size:10}}}},
      xaxis:{{tickformat:'%d/%m',tickfont:{{size:9}},showgrid:false,dtick:'D7'}},
      yaxis:{{showgrid:true,gridcolor:'#F0F0F0',zeroline:false,tickfont:{{size:9}},rangemode:'tozero'}},
      margin:{{l:32,r:12,t:38,b:48}},
    }},CONFIG);
  }}

  // ── Chart 2: Acurácia por Dia da Semana ────────────────────────────────────
  const elDia = document.getElementById('pred-chart-dia');
  if(elDia) {{
    const diasAcur = DIAS_ORD.map(d=>{{
      const dp = valid.filter(p=>p.dia===d);
      if(dp.length===0) return null;
      return {{dia:d, pct:Math.round(dp.filter(p=>p.acerto).length/dp.length*100), n:dp.length,
               med:(dp.reduce((s,p)=>s+p.actual,0)/dp.length).toFixed(1)}};
    }}).filter(Boolean);

    const sub = document.getElementById('pred-subtit-dia');
    if(sub) sub.textContent = '% de dias onde a ocorrência real caiu dentro do intervalo previsto';

    Plotly.react('pred-chart-dia',
      [{{type:'bar', x:diasAcur.map(d=>d.dia), y:diasAcur.map(d=>d.pct),
         text:diasAcur.map(d=>d.pct+'%'), textposition:'outside', textfont:{{size:10,color:'#333'}},
         marker:{{color:diasAcur.map(d=>d.pct>=70?COR_ACERTO:d.pct>=50?'rgba(224,123,0,0.75)':COR_ERRO)}},
         hovertemplate:'<b>%{{x}}</b><br>Acurácia: %{{y}}%<br>Média real: %{{customdata}} oc./dia<extra></extra>',
         customdata:diasAcur.map(d=>d.med),
      }}],
      {{...LAYOUT_BASE,
        xaxis:{{tickfont:{{size:10}},showgrid:false}},
        yaxis:{{showgrid:true,gridcolor:'#F0F0F0',zeroline:false,tickfont:{{size:9}},range:[0,115],ticksuffix:'%'}},
        margin:{{l:32,r:12,t:12,b:40}},
        shapes:[{{type:'line',x0:-0.5,x1:DIAS_ORD.length-0.5,y0:70,y1:70,
          line:{{color:'#107C10',width:1.5,dash:'dot'}}}}],
        annotations:[{{x:0,y:72,text:'Meta 70%',showarrow:false,font:{{size:9,color:'#107C10'}},xref:'paper',yref:'y'}}],
      }},CONFIG);
  }}

  // ── Chart 3: Heatmap Dia × Turno ────────────────────────────────────────────
  const elHeat = document.getElementById('pred-chart-heatmap');
  if(elHeat) {{
    const mat = {{}};
    data.forEach(r=>{{ if(r.dia&&r.turno){{ const k=r.dia+'||'+r.turno; mat[k]=(mat[k]||0)+1; }} }});
    const zMat   = DIAS_ORD.map(d=>TURNS.map(t=>mat[d+'||'+t]||0));
    const maxZ   = Math.max(...zMat.flat(),1);
    const annots = [];
    DIAS_ORD.forEach((d,i)=>TURNS.forEach((t,j)=>{{
      const v = zMat[i][j];
      annots.push({{x:t,y:d,text:v>0?String(v):'',showarrow:false,
        font:{{color:v/maxZ>0.55?'white':'#333',size:11}}}});
    }}));
    Plotly.react('pred-chart-heatmap',[{{
      type:'heatmap',z:zMat,x:TURNS,y:DIAS_ORD,
      colorscale:[[0,'#F5F8FF'],[0.3,'#FFF3CD'],[0.65,'#FFB347'],[1,'#D13438']],
      showscale:true, colorbar:{{thickness:10,len:0.7,tickfont:{{size:9}}}},
      hovertemplate:'<b>%{{y}} — %{{x}}</b><br>%{{z}} ocorrências históricas<extra></extra>',
    }}],{{
      ...LAYOUT_BASE, annotations:annots,
      xaxis:{{side:'top',tickfont:{{size:11,weight:700}},showgrid:false}},
      yaxis:{{tickfont:{{size:10}},automargin:true,showgrid:false}},
      margin:{{l:80,r:50,t:44,b:10}},
    }},CONFIG);
  }}

  // ── Tabela: últimas previsões vs real ────────────────────────────────────────
  const elTab = document.getElementById('pred-tabela');
  if(elTab) {{
    const rec10 = valid.slice(-15).reverse();
    const sub   = document.getElementById('pred-subtit-tab');
    if(sub) sub.textContent = 'As '+rec10.length+' últimas datas com dados suficientes para previsão';
    const rows  = rec10.map(p=>{{
      const ic  = p.acerto ? '✅' : '❌';
      const cl  = p.acerto ? '#f0fff4' : '#fff0f0';
      const err = (p.actual-p.predMed);
      const errTxt = (err>0?'+':'')+err.toFixed(1);
      const topT = p.tipos ? Object.entries(p.tipos).sort((a,b)=>b[1]-a[1])[0] : null;
      return `<tr style="background:${{cl}}">
        <td style="padding:4px 8px;font-size:10px;white-space:nowrap">${{p.date.split('-').reverse().join('/')}}</td>
        <td style="padding:4px 8px;font-size:10px">${{p.dia||'—'}}</td>
        <td style="padding:4px 8px;font-size:10px;text-align:center">${{p.predMin}}–${{p.predMax}}</td>
        <td style="padding:4px 8px;font-size:10px;text-align:center;font-weight:700">${{p.actual}}</td>
        <td style="padding:4px 8px;font-size:10px;text-align:center;color:${{err>0?'#D13438':'#107C10'}}">${{errTxt}}</td>
        <td style="padding:4px 8px;font-size:11px;text-align:center">${{ic}}</td>
        <td style="padding:4px 8px;font-size:10px">${{topT?topT[0]:'—'}}</td>
      </tr>`;
    }}).join('');
    elTab.innerHTML = `<table style="width:100%;border-collapse:collapse;font-size:11px">
      <thead><tr style="background:#F0F4FA;position:sticky;top:0">
        <th style="padding:5px 8px;text-align:left;font-size:10px">Data</th>
        <th style="padding:5px 8px;text-align:left;font-size:10px">Dia</th>
        <th style="padding:5px 8px;text-align:center;font-size:10px">Previsto</th>
        <th style="padding:5px 8px;text-align:center;font-size:10px">Real</th>
        <th style="padding:5px 8px;text-align:center;font-size:10px">Erro</th>
        <th style="padding:5px 8px;text-align:center;font-size:10px">Acerto</th>
        <th style="padding:5px 8px;text-align:left;font-size:10px">+ Frequente</th>
