...
# ── DESTINATÁRIOS ────────────────────────────────────────────────────────────
# Adicione aqui os IDs de quem deve receber as mensagens.
...
if __name__ == '__main__':
    ...
    if '--test' in sys.argv:
        ...
        msg = (
            f"✅ *MENSAGEM DE TESTE — GUARDA MUNICIPAL BC*\n"
            f"📅 {now}\n\n"
            f"Este é um teste de envio automático.\n"
            f"Se você recebeu esta mensagem, está tudo configurado corretamente!"
        )
        print("\nEnviando mensagem de teste...")
        for dest in DESTINATARIOS:
            print(f"  -> Enviando para {dest}...")
            enviar_telegram(BOT_TOKEN, dest, msg)
        print("\nTeste concluido.")
        sys.exit(0)
